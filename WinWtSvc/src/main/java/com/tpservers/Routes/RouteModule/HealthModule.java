...
public final class HealthModule {
    ...
    public static void healthHeartbeat(JsonObject root) {
        CommandDispatcher.send(root.get("route_host_identifier").getAsString(), "health", "heartbeat", "");
    }
    ...
    public static void healthNetwork(JsonObject root) {
        ...
        if (root.has("payload_read")) {
            ...
            if (payload.has("ping_count"))
                args.add("ping_count", payload.get("ping_count"));

            if (payload.has("targets"))
                args.add("targets", payload.get("targets"));
        }

        CommandDispatcher.send(
                root.get("route_host_identifier").getAsString(),
                "health",
                "health-network",
                args.toString());
    }
